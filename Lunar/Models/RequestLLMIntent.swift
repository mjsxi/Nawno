...
struct RequestLLMIntent: AppIntent {
    ...
    static let description: LocalizedStringResource = "start a new chat"
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        ...
        if prompt.isEmpty {
            if let output = thread.messages.last?.content {
                ...
                throw $prompt.needsValueError("chat") // re-prompt
            }
        }
        ...
        if let modelName = appPreferences.currentModelName {
            ...
            if continuous {
                throw $prompt.needsValueError("\(output)") // re-prompt infinitely until user cancels
            }
